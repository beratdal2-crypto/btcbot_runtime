...
def main() -> None:
    ...
    with apply_research_profile("btc_5m_breakout"):
    ...
    trades_df.to_csv("logs/quick_btc_5m_breakout_backtest.csv", index=False)
    pd.DataFrame([wf_summary]).to_csv("logs/quick_btc_5m_breakout_walkforward.csv", index=False)
    payload = {
        "backtest": bt_summary,
    ...
    with open("logs/quick_btc_5m_breakout_check.json", "w") as f:
