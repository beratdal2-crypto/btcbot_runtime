...
RESULTS_PATH = "logs/eth_pullback_sweep.csv"
BEST_PATH = "logs/eth_pullback_best.json"
...
def main() -> pd.DataFrame:
    ...
    grid = {
        "entry_score_threshold": [0.53, 0.55, 0.57],
        "entry_min_volume_ratio": [0.90, 0.95, 1.00],
        "mean_reversion_prob_floor": [0.57, 0.59, 0.61],
        "mean_reversion_min_model_prob_up": [0.42, 0.44, 0.46],
    }
    ...
    for entry_score_threshold, entry_min_volume_ratio, prob_floor, model_prob_floor in product(
        ...
        with _temporary_settings(overrides):
            _, bt_summary = run_backtest(
                profile_name="eth_5m_pullback",
                symbols=("ETHUSDT",),
                trades_path="logs/eth_pullback_grid_backtest_trades.csv",
                summary_path="logs/eth_pullback_grid_backtest_summary.csv",
                refresh_scores=False,
            )
            wf_df = run_walkforward(
                profile_name="eth_5m_pullback",
                symbols=("ETHUSDT",),
                output_path="logs/eth_pullback_grid_walkforward.csv",
                refresh_scores=False,
            )
